	...
	async execute(message, args) {
		...
		if (role.permissions.has('Administrator') || role.permissions.has('ManageGuild')) {
			const container = buildNotice(
				...
				`I cannot assign roles with **Administrator** or **Manage Server** permissions. Please adjust ${role.name} manually.`
			);
			return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2, allowedMentions: { repliedUser: false } });
		}
		...
		if (targetMember.roles.cache.has(role.id)) {
			const container = buildNotice(
				`# ${EMOJIS.error} Already Assigned`,
				`${targetMember.user.username} already has **${role.name}**.`
			);
			return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2, allowedMentions: { repliedUser: false } });
		}
		...
		try {
			...
			const container = buildNotice(
				`# ${EMOJIS.error} Error`,
				'Could not assign the role. Please try again.'
			);
			return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2, allowedMentions: { repliedUser: false } });
		}
	}
