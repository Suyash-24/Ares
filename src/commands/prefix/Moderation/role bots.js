...
export default {
	...
	async execute(message, args, client) {
		...
		if (!args.length) {
			return message.reply({
				components: [buildNotice(`# ${EMOJIS.error} Missing Arguments`, `Usage: \`role bots <role>\``)],
				flags: MessageFlags.IsComponentsV2,
				allowedMentions: { repliedUser: false }
			});
		}
		...
		await processingMsg.edit({
			components: [buildNotice(`# ${EMOJIS.loading} Processing`, `Adding role to ${bots.size} bot(s)...`)]
		});
		...
		for (const member of bots.values()) {
			...
			try {
				markCommandInvoker(message.guild.id, 'rolebots', member.id, message.author);
		...
		if (affected > 0) {
			await sendLog(message.client, message.guildId, LOG_EVENTS.MOD_MASS_ACTION, {
				...
				action: 'Role Add to Bots',
				count: affected,
				role: role,
				details: `Added ${role.name} to ${affected} bot(s)${failures > 0 ? `, ${failures} failed` : ''}`
			});
		}
		...
		return processingMsg.edit({
			components: [
				buildNotice(
					`# ${EMOJIS.success} Role Added to Bots`,
					`**Role:** ${formatRole(role)}\n**Affected:** ${affected}\n**Failures:** ${failures}`
				)
			]
		});
