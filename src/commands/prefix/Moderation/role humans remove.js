...
export default {
	...
	async execute(message, args, client) {
		...
		if (!args.length) {
			return message.reply({
				components: [buildNotice(`# ${EMOJIS.error} Missing Arguments`, `Usage: \`role humans remove <role>\``)],
				flags: MessageFlags.IsComponentsV2,
				allowedMentions: { repliedUser: false }
			});
		}
		...
		await processingMsg.edit({
			components: [buildNotice(`# ${EMOJIS.loading} Processing`, `Removing role from ${humans.size} member(s)...`)]
		});
		...
		for (const member of humans.values()) {
			...
			try {
				markCommandInvoker(message.guild.id, 'rolehumansremove', member.id, message.author);
		...
		if (affected > 0) {
			await sendLog(message.client, message.guildId, LOG_EVENTS.MOD_MASS_ACTION, {
				...
				action: 'Role Remove from Humans',
				count: affected,
				role: role,
				details: `Removed ${role.name} from ${affected} human(s)${failures > 0 ? `, ${failures} failed` : ''}`
			});
		}
		...
		return processingMsg.edit({
			components: [
				buildNotice(
					`# ${EMOJIS.success} Role Removed from Humans`,
					`**Role:** ${formatRole(role)}\n**Affected:** ${affected}\n**Failures:** ${failures}`
				)
			]
		});
