	...
	async execute(message, args) {
		if (!args.length) {
			return message.reply({
				components: [buildNotice(`# ${EMOJIS.error} Missing Arguments`, `Usage: \`role hoist <role>\``)],
		...
		try {
			markCommandInvoker(message.guild.id, 'rolehoist', role.id, message.author);
			const newHoist = !role.hoist;
			await role.edit({ hoist: newHoist });
			const status = newHoist ? 'Enabled' : 'Disabled';
			return message.reply({
				components: [
					buildNotice(
						`# ${EMOJIS.success} Hoist ${status}`,
