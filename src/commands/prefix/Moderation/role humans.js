...
export default {
	...
	async execute(message, args, client) {
		...
		if (!args.length) {
			return message.reply({
				components: [buildNotice(`# ${EMOJIS.error} Missing Arguments`, `Usage: \`role humans <role>\``)],
				flags: MessageFlags.IsComponentsV2,
				allowedMentions: { repliedUser: false }
			});
		}
		...
		await processingMsg.edit({
			components: [buildNotice(`# ${EMOJIS.loading} Processing`, `Adding role to ${humans.size} member(s)...`)]
		});
		...
		for (const member of humans.values()) {
			...
			try {
				markCommandInvoker(message.guild.id, 'rolehumans', member.id, message.author);
		...
		// Send mod log for mass role add
		if (affected > 0) {
			await sendLog(message.client, message.guildId, LOG_EVENTS.MOD_MASS_ACTION, {
				executor: message.author,
				action: 'Role Add to Humans',
				count: affected,
				role: role,
				details: `Added ${role.name} to ${affected} human(s)${failures > 0 ? `, ${failures} failed` : ''}`
			});
		}
		...
		return processingMsg.edit({
			components: [
				buildNotice(
					`# ${EMOJIS.success} Role Added to Humans`,
					`**Role:** ${formatRole(role)}\n**Affected:** ${affected}\n**Failures:** ${failures}`
				)
			]
		});
