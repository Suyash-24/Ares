...
const name = 'vcdeafenall';
const aliases = ['deafenall', 'vdeafenall'];
const description = 'Server deafens all members in the current voice channel.';
const usage = 'vcdeafenall';
...
async function execute(message, args, client) {
	...
	// Filter members: not bot, not already deafened, not the author
	const membersToDeafen = channel.members.filter(m => !m.user.bot && !m.voice.serverDeaf && m.id !== message.author.id);

	if (membersToDeafen.size === 0) {
		container.addTextDisplayComponents(td => 
			td.setContent(`${EMOJIS.info || 'ℹ️'} There are no members to deafen in **${channel.name}**.`)
		);
		return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2, allowedMentions: { repliedUser: false, parse: [] } });
	}
	...
	// Process in parallel
	const promises = membersToDeafen.map(async (member) => {
		// Hierarchy check
		...
		try {
			await member.voice.setDeaf(true, `Deafen all by ${message.author.tag}`);
			successCount++;
		} catch (e) {
			failCount++;
		}
	...
	let content = `${EMOJIS.success || '✅'} Server deafened **${successCount}** members in **${channel.name}**.`;
	if (failCount > 0) {
		content += `\n${EMOJIS.error || '❌'} Failed to deafen **${failCount}** members due to permissions/hierarchy.`;
	}
