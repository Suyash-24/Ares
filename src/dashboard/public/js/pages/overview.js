    ...
    document.getElementById('page-content').innerHTML = `
      ...
      <div class="grid grid-2">
        <div class="card">
          ...
          <table>
            <tr><td style="color:var(--text-dim)">Channels</td><td>${guild.channels}</td></tr>
            <tr><td style="color:var(--text-dim)">Roles</td><td>${guild.roles}</td></tr>
            <tr><td style="color:var(--text-dim)">Boost Level</td><td>Tier ${guild.boostLevel}</td></tr>
            <tr><td style="color:var(--text-dim)">Boosts</td><td>${guild.boostCount}</td></tr>
            <tr><td style="color:var(--text-dim)">Active Users</td><td>${stats.activeUsers || 0}</td></tr>
            <tr><td style="color:var(--text-dim)">Active Channels</td><td>${stats.activeChannels || 0}</td></tr>
          </table>
        </div>
        ...
        <div class="card">
          ...
          ${enabledModules.length
            ? enabledModules.map(m => `<span class="badge badge-accent" style="margin: 4px 4px 4px 0;">${m}</span>`).join('')
            : '<p style="color:var(--text-dim); font-size: 0.88rem;">No modules enabled yet. Head to the config page to set them up.</p>'
          }
        </div>
      </div>
    `;
